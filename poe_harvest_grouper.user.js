...
// @version      0.3.6
// @description  Paste items from PoE to instantly search trade. Auto-detects harvest-swappable elemental stats and offers one-click count-group broadening.
// @author       ShaneIsley
...
(function () {
    ...
    // builds swap-group lookups AND regex patterns for item paste matching
    // =========================================================================
    ...
    const StatResolver = {
        ...
        patterns: [],                 // { regex, stats: [{id, type}], text } for item paste matching
        loaded: false,
        ...
        _index(data) {
            ...
            // Step 3: Build regex patterns for item paste matching
            this._buildPatterns();
        },

        // Build regex patterns from trade API stat text templates
        _buildPatterns() {
            this.patterns = [];

            // Group stat IDs by text template (same text, different namespaces)
            const templateMap = new Map(); // text → [{id, type}]
            this.idToMeta.forEach((meta, id) => {
                if (!templateMap.has(meta.text)) templateMap.set(meta.text, []);
                templateMap.get(meta.text).push({ id, type: meta.type });
            });

            // Convert each template to a regex
            templateMap.forEach((stats, text) => {
                // Skip pseudo stats — they're calculated, not on items
                if (stats.every(s => s.type === 'pseudo')) return;

                let rx = text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // escape special chars
                rx = rx.replace(/\\\+/g, '\\+?');                      // make + optional
                rx = rx.replace(/#/g, '([+-]?\\d+(?:\\.\\d+)?)');      // # → number capture
                rx = rx.replace(/\s+/g, '\\s+');                       // flexible whitespace
                ...
                } catch (e) { /* invalid regex from unusual stat text — skip */ }
            });

            // Sort: longer templates first (more specific patterns match before shorter ones)
            this.patterns.sort((a, b) => b.text.length - a.text.length);
        },

        // Match a mod line against patterns, returning the best stat ID for the given source
        matchMod(modText, source) {
            for (const p of this.patterns) {
                const m = modText.match(p.regex);
                if (m) {
                    // Pick the stat ID matching the source namespace, with fallbacks
                    const byType = {};
                    p.stats.forEach(s => { byType[s.type] = s.id; });

                    let id;
                    switch (source) {
                        case 'implicit': id = byType.implicit || byType.explicit; break;
                        case 'enchant':  id = byType.enchant  || byType.implicit; break;
                        case 'crafted':  id = byType.crafted  || byType.explicit; break;
                        default:         id = byType.explicit  || byType.crafted;  break;
                    }
                    if (!id) id = p.stats[0].id; // absolute fallback

                    // Extract the first numeric value for use as min filter
                    const value = m[1] ? parseFloat(m[1]) : undefined;

                    return { id, value, text: p.text, source, values: Array.from(m).slice(1).map(Number) };
                }
            }
            return null;
        },
    ...
    const ItemParser = {
        parse(text) {
            ...
            const propRx = /^(Armour|Evasion|Energy Shield|Physical Damage|Elemental Damage|Attacks per Second|Critical Strike|Weapon Range|Charm Slots|Ward):/;
            ...
            for (let i = 1; i < sections.length; i++) {
                ...
                for (const line of lines) {
                    ...
                    // Normalize ranges: (10-20) → average (shouldn't appear in game copy, but just in case)
    ...
    const PasteSearch = {
        ...
        init() {
            // Overlay is created on first open, to keep DOM clean
        },
        ...
        _createOverlay() {
            ...
            textarea.addEventListener('paste', () => {
                // Delay to let paste populate the textarea
                setTimeout(() => this._parseAndRender(), 50);
            });
            // Also parse on input (manual typing/edit)
        ...
        async _doSearch() {
            ...
            this.overlay.querySelectorAll('[data-ps-mod]').forEach(cb => {
                if (cb.checked) {
                    ...
                    if (matched[idx]) checkedMods.push(matched[idx]);
                }
            });
            ...
            try {
                ...
                if (resp.status === 429) {
                    ...
                    searchBtn.disabled = false;
                    searchBtn.textContent = `⏳ Rate limited — ${retry}s`;
                    return;
                }
                if (!resp.ok) {
                ...
                if (data.id) {
                    ...
                    searchBtn.disabled = false;
                    searchBtn.textContent = '❌ No search ID';
                }
            } catch (e) {
                searchBtn.disabled = false;
                searchBtn.textContent = '❌ Network error';
            } finally {
    ...
    const Sidebar = {
        ...
        init() {
            ...
            style.textContent = `
                ...
                .ps-mod-row{display:flex;align-items:center;gap:6px;padding:4px 6px;border-radius:3px;font-size:12px;color:#c8c8c8;cursor:pointer}
        ...
        // ── Core analysis (harvest grouping) ──
        analyze(payload, league) {
            ...
            // Dedup: keep best per swap group
        ...
        _render(league) {
            ...
            this.panel.querySelector('.hg-status').innerHTML = count > 1
                ? `${count} groups · <a href="#" data-hg-apply-all>Apply all</a>` : `${count} group available`;
        ...
        _renderBar(league) {
            ...
            const count = this.suggestions.length;
            this.barBadge.textContent = count;
            this.barBadge.classList.toggle('hg-zero', count === 0);
            if (count === 0) { this.barBody.innerHTML = '<div class="hg-bar-empty">No swaps detected.</div>'; return; }
            ...
            if (count > 1) html += '<div class="hg-bar-footer"><button class="hg-bar-go" data-hg-bar-all>▶ Apply All</button></div>';
        ...
        // ── Payload builder + search submission ──
        _buildGroupedPayload(suggestions) {
            const payload = JSON.parse(JSON.stringify(_lastPayload));
            const idsToDisable = new Set();
            suggestions.forEach(sug => sug.detected.forEach(d => idsToDisable.add(d.id)));
            (payload.query.stats || []).forEach(sg => (sg.filters || []).forEach(f => { if (idsToDisable.has(f.id)) f.disabled = true; }));
        ...
        async _applyAll(league) {
            ...
            this.content.querySelectorAll('.hg-apply').forEach(b => { b.disabled = true; b.textContent = 'Searching…'; });
            this.barBody.querySelectorAll('.hg-bar-go').forEach(b => { b.disabled = true; });
            await this._submitSearch(this._buildGroupedPayload(this.suggestions), league || detectLeague(),
                this.content.querySelector('.hg-apply') || this.barBody.querySelector('.hg-bar-go'));
        },
